...
/*
			...
			clock_t time_s2 = clock();	
			bool left_flag = ransac.ransac2(_left_pt_set,left_best_model,n_max,n_sample,thre,left_inliner_flag);
			bool right_flag =ransac.ransac2(_right_pt_set,right_best_model,n_max,n_sample,thre,right_inliner_flag);

			cout << "ransac time is :" << 1000*(clock()-time_s2)/(double)CLOCKS_PER_SEC << "ms" << endl;
...
//				cout <<  "left_best_model" << endl;
//				cout <<  left_best_model.at<double>(0,0)  << endl;
//				cout <<  left_best_model.at<double>(1,0)  << endl;
//				cout <<  left_best_model.at<double>(2,0)  << endl;
...
//				cout <<  "right_best_model" << endl;
//				cout <<  right_best_model.at<double>(0,0)  << endl;
//				cout <<  right_best_model.at<double>(1,0)  << endl;
//				cout <<  right_best_model.at<double>(2,0)  << endl;
			...
			*/
			...
			srand((unsigned int)time(NULL)); //设置随机数种子
			ransac_fit ransac;
			cv::Mat left_best_model(2,1,CV_64FC1);//曲线模型
			cv::Mat right_best_model(2,1,CV_64FC1);//曲线模型
			int n_max = 50;//最大迭代次数
			int n_sample = 10;//采样数目
			double thre = 10;//内点阈值
			vector<bool> left_inliner_flag;//内点标志
			...
			//判断是否拟合出了直线呀
			if((left_best_model.at<double>(1,0)>0)&& (left_best_model.at<double>(0,0)<0))
			{
				bl= left_best_model.at<double>(1,0);
				cl= left_best_model.at<double>(0,0);
//				cout <<  "left_best_model" << endl;
...
//				cout <<  left_best_model.at<double>(2,0)  << endl;
			...
			if((right_best_model.at<double>(1,0)<0)&& (right_best_model.at<double>(0,0)>0))
			{
				br=right_best_model.at<double>(1,0);
				cr=right_best_model.at<double>(0,0);
//				cout <<  "right_best_model" << endl;
...
//				cout <<  right_best_model.at<double>(2,0)  << endl;
			...
			for (double y = 440; y < img.rows; y++) 
			{ 
				double x_left = left_best_model.at<double>(1,0)+left_best_model.at<double>(0,0)*y;
				points_fitted_left.push_back(cv::Point(x_left, y));
				double x_right = right_best_model.at<double>(1,0)+right_best_model.at<double>(0,0)*y;
				points_fitted_right.push_back(cv::Point(x_right, y));
			}
